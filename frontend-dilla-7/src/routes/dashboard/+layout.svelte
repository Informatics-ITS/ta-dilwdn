<script lang="ts">
  ...
  function getTitle(path: string) {
    ...
    if (path.startsWith('/dashboard/kelas')) return 'Kelas'
    if (path.startsWith('/dashboard/ujian')) return 'Ujian'
    if (path.startsWith('/dashboard/laporan')) return 'Laporan'
    if (path.startsWith('/dashboard/analisis')) return 'Analisis'
    return 'Dashboard'
  }
...
<div class="flex h-screen bg-[#f9f9f9] text-gray-900">
  ...
  >
    ...
    <nav class="flex flex-col gap-4">
      <a href="/dashboard" class={`flex items-center gap-3 px-3 py-2 font-semibold rounded-full ${currentPath === '/dashboard' ? 'bg-primary text-white' : 'hover:bg-purple-100'}`}>
        <Menu class="w-5 h-5" />
        Beranda
      </a>

      <a href="/dashboard/kelas" class={`flex items-center gap-3 px-3 py-2 font-semibold rounded-full ${currentPath.startsWith('/dashboard/kelas') ? 'bg-primary text-white' : 'hover:bg-purple-100'}`}>
        <Users class="w-5 h-5" />
        Kelas
      </a>

      <a href="/dashboard/ujian" class={`flex items-center gap-3 px-3 py-2 font-semibold rounded-full ${currentPath.startsWith('/dashboard/ujian') ? 'bg-primary text-white' : 'hover:bg-purple-100'}`}>
      ...
      <a
        ...
        class={`flex items-center gap-3 px-3 py-2 font-semibold rounded-full ${currentPath.startsWith('/dashboard/laporan') ? 'bg-primary text-white' : 'hover:bg-purple-100'}`}
      >
        <FileText class="w-5 h-5" />
        Laporan
      </a>
      ...
      <a
        ...
        class={`flex items-center gap-3 px-3 py-2 font-semibold rounded-full ${currentPath.startsWith('/dashboard/analisis') ? 'bg-primary text-white' : 'hover:bg-purple-100'}`}
      >
